...
class System extends Base
{
    ...
    public function configapppopupwindow()
    {
        if (Request()->isPost()) {
            $config = input();
        ...
        return $this->fetch('admin@system/configapppopupwindow');
    }

    public function configtaskarticle()
    {

        if (Request()->isPost()) {

            $config_old = config('task');
            $config = input();
            $config_new['article']['title'] = $config['title'];
            $config_new['article']['status'] = $config['status'];


            // 签到配置
            $config_new['sign']['status'] = $config['sign_status'];
            $config_new['sign']['reward']['points'] = $config['sign_points'];
            $config_new['sign']['title'] = $config_old['sign']['title'];
            $config_new['sign']['info'] = $config_old['sign']['info'];

            // 评论
            $config_new['comment']['status'] = $config['comment_status'];
            $config_new['comment']['reward']['points'] = $config['comment_points'];
            $config_new['comment']['reward_num'] = $config['comment_reward_num'];
            $config_new['comment']['title'] = $config_old['comment']['title'];
            $config_new['comment']['info'] = $config_old['comment']['info'];

            //点赞
            $config_new['dianzan']['status'] = $config['dianzan_status'];
            $config_new['dianzan']['reward']['points'] = $config['dianzan_points'];
            $config_new['dianzan']['title'] = $config_old['dianzan']['title'];
            $config_new['dianzan']['info'] = $config_old['dianzan']['info'];

            //评分
            $config_new['mark']['status'] = $config['mark_status'];
            $config_new['mark']['reward']['points'] = $config['mark_points'];
            $config_new['mark']['reward_num'] = $config['mark_reward_num'];
            $config_new['mark']['title'] = $config_old['mark']['title'];
            $config_new['mark']['info'] = $config_old['mark']['info'];

            //弹幕
            $config_new['danmu']['status'] = $config['danmu_status'];
            $config_new['danmu']['reward']['points'] = $config['danmu_points'];
            $config_new['danmu']['reward_num'] = $config['danmu_reward_num'];
            $config_new['danmu']['title'] = $config_old['danmu']['title'];
            $config_new['danmu']['info'] = $config_old['danmu']['info'];

            //分享
            $config_new['share']['status'] = $config['share_status'];
            $config_new['share']['reward']['points'] = $config['share_points'];
            $config_new['share']['reward_num'] = $config['share_reward_num'];
            $config_new['share']['title'] = $config_old['share']['title'];
            $config_new['share']['info'] = $config_old['share']['info'];


            $config_new = array_merge($config_old, $config_new);

            $res = mac_arr2file(APP_PATH . 'extra/task.php', $config_new);
            if ($res === false) {
                return $this->error('保存失败，请重试!');
            }
            return $this->success('保存成功!');
        }
        $this->assign('config', config('task'));
        return $this->fetch('admin@system/configtaskarticle');
    }
