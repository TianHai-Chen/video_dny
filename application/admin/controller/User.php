...
class User extends Base
{
    ...
    public function reward()
    {
        ...
        $where3=[];
        $user_ids_1 = empty($user_ids_1) ? -1 : $user_ids_1;
        ...
        $where3=[];
        $user_ids_2 = empty($user_ids_2) ? -1 : $user_ids_2;
        ...
        $where3=[];
        $user_ids_3 = empty($user_ids_3) ? -1 : $user_ids_3;
        $where3['user_id_1'] = ['in',$user_ids_3];
        $points_cc_3 = Db::name('Plog')->where($where3)->sum('plog_points');

        // print_r($param['uid']);exit;
        $points_sum = Db::name('Plog')->where(['user_id' => $param['uid'], 'plog_type' => 4])->sum('plog_points');
        ...
        $data['points_sum'] = intval($points_sum);
    ...
    public function grant() {
        ...
        foreach ($data as $v) {
            model('Plog')->where('plog_id', $v['plog_id'])->update(['plog_type' => 4]);
