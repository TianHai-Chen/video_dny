...
public function postBuypopedom()
    {
        ...
        if ($param['pay_type'] == 1 && $config_user['trysee_points'] > $GLOBALS['user']['user_points']) {
            return $this->error('对不起,观看本片需要支付[' . $data['ulog_points'] . ']仙豆，您还剩下[' . $GLOBALS['user']['user_points'] . ']仙豆，剩余仙豆不足以观看本片,请先充值或者完成激励任务获取仙豆！');
        } else {
            switch ($param['pay_type']){
                case 1:
                    ...
                    if ($res === false) {
                        return $this->error('对不起,更新用户仙豆信息失败，请刷新重试！');
                    }
...
//        $this->assign('param',$param);
     ...
     * 判断用户仙豆是否达到变更代理的仙豆值，不够则提示，够的话则继续；
     * 扣除仙豆，变更用户身份标识
     *
     * */
     ...
     * 后台设置点播需要的仙豆，根据后台设置的收费模式（每集、每视频）进行处理
     * */
    public function getCheckVodTrySee()
    {
        ...
        if(intval($vod_points_play)>0 and !empty($user)){
            ...
            if(count($ulog)>0){
                ...
                $info = ['code'=>1,'msg'=>'您需要花费'.$vod_points_play.'仙豆才能观看','status'=>2,'trysee'=>$trysee];
            }
        }
    ...
    public function getPayTip()
    {
        ...
        $url = $pay['card']['url'];
        $data = ['url'=>$url];
        return $this->success($data,'最小充值金额为'.$min.'元，1元可兑换'.$scale.'个仙豆');
    }
