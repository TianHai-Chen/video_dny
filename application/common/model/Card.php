...
//        }
        ...
        // 只验证密码
        if (empty($card_pwd) || empty($user_info)) {
            return ['code' => 1001, 'msg' => '参数错误'];
        }
...
//        $where['card_no'] = ['eq',$card_no];
        ...
        if($config['user']['reward_status']=='1'){
            $ratio = $config['user']['reward_ratio'];
            $ratio_2 = $config['user']['reward_ratio_2'];
            $ratio_3 = $config['user']['reward_ratio_3'];

            $cur_user = model('User')->where($where2)->field('user_pid,user_pid_2,user_pid_3')->find();

            $user_pid = $cur_user['user_pid'];
            $user_1 = model('User')->where('user_id',$user_pid)->find();
            if(!empty($user_1)){
                if($user_1['is_agents']==1){
                    model('User')->where('user_id',$user_pid)->setInc('user_gold',intval($info['card_points']*$ratio));
                }else{
                    model('User')->where('user_id',$user_pid)->setInc('user_points',intval($info['card_points']*$ratio));
                }
            }
            $user_pid_2 = $cur_user['user_pid_2'];
            $user_2 = model('User')->where('user_id',$user_pid_2)->find();
            if(!empty($user_2)){
                if($user_2['is_agents']==1){
                    model('User')->where('user_id',$user_pid_2)->setInc('user_gold',intval($info['card_points']*$ratio_2));
                }else{
                    model('User')->where('user_id',$user_pid_2)->setInc('user_points',intval($info['card_points']*$ratio_2));
                }
            }
            $user_pid_3 = $cur_user['user_pid_3'];
            $user_3 = model('User')->where('user_id',$user_pid_3)->find();
            if(!empty($user_3)){
                if($user_3['is_agents']==1){
                    model('User')->where('user_id',$user_pid_3)->setInc('user_gold',intval($info['card_points']*$ratio_3));
                }else{
                    model('User')->where('user_id',$user_pid_3)->setInc('user_points',intval($info['card_points']*$ratio_3));
                }
            }
