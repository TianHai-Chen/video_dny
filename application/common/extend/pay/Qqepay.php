...
class Qqepay {
    ...
    public function submit($user,$order,$param){
        ...
        /**************************请求参数**************************/
        $notify_url = $GLOBALS['http_type'] . $_SERVER['HTTP_HOST'] . '/api.php/v1.payment/notify/pay_type/qqepay';
        //需http://格式的完整路径，不能加?id=123这类自定义参数        //页面跳转同步通知页面路径
        $return_url = $GLOBALS['http_type'] . $_SERVER['HTTP_HOST'] . '/index.php/payment/notify/pay_type/qqepay';
        //需http://格式的完整路径，不能加?id=123这类自定义参数，不能写成http://localhost/        //商户订单号
        $out_trade_no = $order['order_code'];
        //商户网站订单系统中唯一订单号，必填
        //支付方式
        $pay_type = $GLOBALS['config']['pay']['qqepay']['type'];
