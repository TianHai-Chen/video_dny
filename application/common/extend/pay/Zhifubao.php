...
class Zhifubao {
    ...
    public function submit($user,$order,$param){
        ...
        /**************************请求参数**************************/
        $notify_url = $GLOBALS['http_type'] . $_SERVER['HTTP_HOST'] . '/api.php/v1.payment/notify/pay_type/zhifubao';
        //需http://格式的完整路径，不能加?id=123这类自定义参数        //页面跳转同步通知页面路径
        $return_url = $GLOBALS['http_type'] . $_SERVER['HTTP_HOST'] . '/index.php/payment/notify/pay_type/zhifubao';
        //需http://格式的完整路径，不能加?id=123这类自定义参数，不能写成http://localhost/        //商户订单号
        $out_trade_no = $order['order_code'];
        //商户网站订单系统中唯一订单号，必填
        //支付方式
        $pay_type = $GLOBALS['config']['pay']['zhifubao']['type'];
        ...
        //商品名称
        $name = '用户'.$user['user_nick_name'].'的VIP会员特权';
        //付款金额
        ...
        //必填        //订单描述
        /************************************************************/
        $alipay_config['partner'] = $GLOBALS['config']['pay']['zhifubao']['appid'];
        $alipay_config['key'] = $GLOBALS['config']['pay']['zhifubao']['appkey'];
        $alipay_config['sign_type']    = strtoupper('MD5');
        $alipay_config['input_charset']= strtolower('utf-8');
        $alipay_config['transport'] = $GLOBALS['http_type'];
        $alipay_config['apiurl'] = $GLOBALS['config']['pay']['zhifubao']['apiurl'];
//构造要请求的参数数组，无需改动
        $parameter = array(
            "pid" => trim($GLOBALS['config']['pay']['zhifubao']['appid']),
            "type" => $type,
    ...
    public function notify(){
        ...
        if($verify_result) {//验证成功
            ...
            if ($_GET['trade_status'] == 'TRADE_SUCCESS') {
                ...
                //如果有做过处理，不执行商户的业务程序
                $res = model('Order')->notify($out_trade_no,'zhifubao');
