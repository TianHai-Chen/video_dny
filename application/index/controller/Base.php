    ...
    protected function check_user_popedom($type_id,$popedom,$param=[],$flag='',$info=[],$trysee=0)
    {
        ...
        elseif($popedom==2 && $flag=='art'){
            ...
            elseif($group['group_id']<3 && $points>0  ){
                ...
                if($res['code'] > 1) {
                    return ['code'=>3003,'msg'=>'观看此数据，需要支付【'.$points.'】仙豆，确认支付吗？','points'=>$points,'confirm'=>1,'trysee'=>0];
                }
            }
        }
        elseif($popedom==3){
            ...
            if($popedom == 4){
                ...
                elseif($group['group_id'] ==2 && $points>0){
                    ...
                    if($res['code'] > 1) {
                        return ['code'=>4003,'msg'=>'下载此数据，需要支付【'.$points.'】仙豆，确认支付吗？','points'=>$points,'confirm'=>1,'trysee'=>0];
                    }
                }
            }
            elseif($popedom==5){
                if(empty($group['group_popedom'][$type_id][3]) && !empty($group['group_popedom'][$type_id][5])){
                    ...
                    elseif( $group['group_id'] <=2 && $points <= intval($user['user_points']) ){
                        return ['code'=>5001,'msg'=>'试看结束,是否支付[' . $points . ']仙豆观看完整数据？您还剩下[' . $user['user_points'] . ']仙豆，请先充值！','trysee'=>$trysee];
                    }
                    elseif( $group['group_id'] <3 && $points > intval($user['user_points']) ){
                        return ['code'=>5002,'msg'=>'对不起,观看此页面数据需要[' . $points . ']仙豆，您还剩下[' . $user['user_points'] . ']仙豆，请先充值！','trysee'=>$trysee];
                    }
                }
            }
        }
