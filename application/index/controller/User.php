...
class User extends Base
{
    ...
    public function ajax_buy_popedom()
    {
        ...
        if ($data['ulog_points'] > $GLOBALS['user']['user_points']) {
            return json(['code' => 2002, 'msg' => '对不起,查看此页面数据需要[' . $data['ulog_points'] . ']仙豆，您还剩下[' . $GLOBALS['user']['user_points'] . ']仙豆，请先充值！']);
        } else {
            $where = [];
            $where['user_id'] = $GLOBALS['user']['user_id'];
            $res = model('User')->where($where)->setDec('user_points',$data['ulog_points']);
            if ($res === false) {
                return json(['code' => 2003, 'msg' => '对不起,更新用户仙豆信息失败，请刷新重试！']);
            }
...
//            model('User')->reward($data['ulog_points']);
    ...
    public function withdraw()
    {
        ...
        if (Request()->isPost()) {
            ...
            $res = model('GoldWithdrawApply')->insert($data);
            return ['code' => $res, 'msg' => '申请成功'];
        }
    ...
    public function junior()
    {
        ...
        $order='user_id desc';
        $res = model('User')->listData($where,$order,$param['page'],$param['limit'],"user_id,user_name,user_nick_name,group_id,user_pid,user_pid_2,user_reg_time");
